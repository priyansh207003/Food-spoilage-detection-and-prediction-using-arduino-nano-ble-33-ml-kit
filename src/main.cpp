...
/* Includes ---------------------------------------------------------------- */
#include <Arduino.h>
#include "food_spoilage_small_project_inferencing.h"
...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        // -------------------------------
        // 🔥 NEW SMART INTERPRETATION LOGIC
        // -------------------------------

        float fresh_score = result.classification[0].value;
        float rotten_score = result.classification[1].value;
        ...
        float MAX_DAYS = 5.0;  // adjust based on food
        float days_left = MAX_DAYS * (1.0 - rotten_score);
        ...
        // Main classification
        if (fresh_score > 0.75) {
            ei_printf("Status: FRESH ✅\n");
            ei_printf("Food is EDIBLE\n");
            ei_printf("Estimated shelf life: %.1f days\n", days_left);
        }
        else if (rotten_score > 0.75) {
            ei_printf("Status: ROTTEN ❌\n");
            ei_printf("Food is NOT EDIBLE\n");
            ei_printf("Spoiled (0 days left)\n");
        }
        else {
            ei_printf("Status: SEMI (Partially spoiled) ⚠️\n");

            if (fresh_score > rotten_score) {
                ei_printf("Food is STILL EDIBLE\n");
                ei_printf("Consume within %.1f days\n", days_left);
            } else {
                ei_printf("Food is NOT SAFE\n");
                ei_printf("Likely to spoil within %.1f days\n", days_left);
            }
        }

        // Extra warning layer
        if (rotten_score >= 0.5 && rotten_score < 0.75) {
            ei_printf("⚠️ WARNING: Food is degrading fast!\n");
        }

        if (rotten_score >= 0.9) {
            ei_printf("🚫 CRITICAL: Completely spoiled!\n");
        }
